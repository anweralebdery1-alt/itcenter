...
## ملاحظات
- **OTP مؤقتاً:** الرمز يظهر على الشاشة ويُملأ تلقائياً (SHOW_OTP=True). عند ربط بوابة SMS لاحقاً اضبط `SHOW_OTP=False`.
- **الصور:** تُخدَم من `/media/` (مهيّأ في الإعدادات).
- **الأمان:** لا تترك `SYNC_API_TOKEN` الافتراضي `changeme_token` — ضع رمزاً قوياً.
